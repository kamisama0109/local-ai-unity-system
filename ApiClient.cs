...
public class ApiClient : MonoBehaviour
{
    ...
    {
        SendChat(message, userId, webSearchConfirmed: false, webSearchAction: null, onSuccess, onError);
    }
    
    public void SendChat(string message, string userId, bool webSearchConfirmed, string webSearchAction, Action<ChatResponse> onSuccess, Action<string> onError)
    {
        StartCoroutine(SendChatCoroutine(message, userId, webSearchConfirmed, webSearchAction, onSuccess, onError));
    }

    private IEnumerator SendChatCoroutine(string message, string userId, bool webSearchConfirmed, string webSearchAction, Action<ChatResponse> onSuccess, Action<string> onError)
    {
        var requestBody = new ChatRequest 
        { 
            message = message, 
            user_id = userId,
            web_search_confirmed = webSearchConfirmed,
            web_search_action = webSearchAction
        };
    ...
    public void GetHistory(string userId, Action<HistoryResponse> onSuccess, Action<string> onError)
    {
        StartCoroutine(GetHistoryCoroutine(userId, onSuccess, onError));
    }

    private IEnumerator GetHistoryCoroutine(string userId, Action<HistoryResponse> onSuccess, Action<string> onError)
    {
        using (var request = UnityWebRequest.Get($"{serverUrl}/api/history?user_id={userId}"))
        {
            request.SetRequestHeader("X-API-Key", apiKey);
            request.timeout = 15;
            yield return request.SendWebRequest();
